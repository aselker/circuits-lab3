...
params = curve_fit(
    lambda Vb, Av, b: Vb * Av + b,
    vb_exp[vals[0]][valid[vals[0]][0] : valid[vals[0]][1]],
    ve_exp[vals[0]][valid[vals[0]][0] : valid[vals[0]][1]],
)
...
ax.plot(
    vb_exp[vals[0]],
    np.array(vb_exp[vals[0]]) * Av + b,
    "k-",
    label="Theoretical Fit no ₘR (Aᵥ = %.2f, V₀ = %.2f)" % (Av, b),
)


for i in range(1, 4):
    ax.plot(
        vb_exp[vals[i]], ve_exp[vals[i]], ".", label="ₘR = " + vals[i][5:], markersize=2
    )
    params = curve_fit(
        lambda Vb, Av, b: Vb * Av + b,
        vb_exp[vals[i]][valid[vals[i]][0] : valid[vals[i]][1]],
        ve_exp[vals[i]][valid[vals[i]][0] : valid[vals[i]][1]],
    )
    Av, b = params[0][0], params[0][1]
    ax.plot(
        vb_exp[vals[i]],
        np.array(vb_exp[vals[i]]) * Av + b,
        "k-",
        label="Theoretical Fit %s (Aᵥ = %.2f, V₀ = %.2f)" % (vals[i][5:], Av, b),
    )
